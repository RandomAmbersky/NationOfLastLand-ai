...
describe('SelectionManager', () => {
  ...
  describe('selectEntity', () => {
    test('должен выбрать сущность, если она еще не выбрана', () => {
      ...
      const mockHandleEntitySelection =
        require('./wasm-imports.js').handle_entity_selection
      mockHandleEntitySelection.mockReturnValue(
        JSON.stringify({
          success: true,
          selected_entities: [1],
  ...
  describe('deselectEntity', () => {
    test('должен отменить выбор сущности', () => {
      ...
      expect(
        mockGameDemo.stateManager.updateSelectionState
  ...
  describe('clearAllSelections', () => {
    test('должен очистить все выделения', () => {
      ...
      mockClearSelection.mockReturnValue(
        JSON.stringify({
          success: true
        })
      )
...
describe('SelectionManager - Additional Coverage Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockGameDemo.selectionIndicatorManager =
      selectionManager.selectionIndicatorManager
  })
  ...
  describe('_showGroupTargetingIndicator', () => {
    test('should show group targeting indicator', () => {
      ...
      expect(
        mockGameDemo.entityRenderer.showTargetIndicator
  ...
  describe('selectAllPlayerUnitsAtBase', () => {
    ...
    test('should show error when no player base found', () => {
      ...
      expect(mockGameDemo.updateStatus).toHaveBeenCalledWith(
        'База игрока не найдена'
      )
    })
  })
  ...
  describe('displayEntityInfo', () => {
    test('should display entity info', async () => {
      ...
      mockGetEntityInfo.mockReturnValue(
        JSON.stringify({
          id: 1,
          vehicle_type: 'scout',
          fraction: 'Player',
    ...
    test('should handle entity not found', async () => {
      ...
      expect(mockGameDemo.updateEntityInfo).toHaveBeenCalledWith(
    ...
    test('should handle error when getting entity info', async () => {
      ...
      expect(mockGameDemo.updateEntityInfo).toHaveBeenCalledWith(
        expect.stringContaining('Error loading entity info')
      )
    })
  })
})
describe('SelectionManager - Additional Coverage Tests Part 2', () => {
  ...
  describe('handleEntityClick', () => {
    ...
    test('should handle successful entity selection with EntitySelected action', async () => {
      ...
      selectionManager._performEntitySelection = jest.fn(() =>
        Promise.resolve({
          success: true,
          action: 'EntitySelected',
          message: 'Selected'
    ...
    test('should handle successful entity selection with GroupTargetAssigned action', async () => {
      ...
      expect(selectionManager._performEntitySelection).toHaveBeenCalled()
      expect(
        selectionManager._showGroupTargetingIndicator
  ...
  describe('_performEntitySelection', () => {
    ...
    test('should perform entity selection without updating local state on failure', async () => {
      ...
      expect(mockHandleEntitySelection).toHaveBeenCalledWith(3, true, [1, 2])
      expect(
        selectionManager._updateLocalSelectionState
  ...
  describe('selectEntity', () => {
    ...
    test('should clear target indicator when selecting a new entity', () => {
      ...
      // Verify
      expect(
        mockGameDemo.entityRenderer.clearTargetIndicator
      ).toHaveBeenCalled()
      expect(result).toBe(true)
    })
  ...
  describe('selectSameTypeUnits', () => {
    ...
    test('should select all player units of same type', () => {
      ...
      // Verify
      expect(selectionManager.clearAllSelections).toHaveBeenCalledWith(true)
      expect(selectionManager.selectEntity).toHaveBeenCalledWith(
        2,
        true,
        false
      )
      ...
      expect(mockGameDemo.updateStatus).toHaveBeenCalledWith(
        'Выделено 2 подвижных юнитов игрока'
      )
    })
  })
