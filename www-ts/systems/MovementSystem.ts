/**
 * Movement System - Handles entity movement and pathfinding
 * Processes move commands from SelectionSystem and updates entity positions
 */

import type { GameEngine, System } from '../core/GameEngine.js'
import type { StateContainer } from '../core/StateContainer.js'

export interface MoveCommand {
  targetX: number
  targetY: number
  speed: number
  distance: number
  progress: number
}

export class MovementSystem implements System {
  public gameEngine: GameEngine
  private moveCommands: Map<number, MoveCommand>
  public isDestroyed: boolean

  constructor (gameEngine: GameEngine) {
    this.gameEngine = gameEngine
    this.moveCommands = new Map() // entityId -> {x, y, speed}
    this.isDestroyed = false
  }

  /**
   * Initialize movement system with Pixi.js app
   * @param app - Pixi.js application
   */
  init (app: PIXI.Application): void {
    // Movement system doesn't need special initialization
    console.log('MovementSystem initialized')
  }

  /**
   * Queue a move command for an entity
   * @param entityId - Entity ID to move
   * @param targetX - Target X coordinate
   * @param targetY - Target Y coordinate
   * @param speed - Movement speed (optional)
   */
  queueMoveCommand (entityId: number, targetX: number, targetY: number, speed: number | null = null): boolean {
    const entity = this.getEntity(entityId)
    if (!entity) {
      console.warn('MovementSystem: Entity not found', entityId)
      return false
    }

    // Entities that can't move
    if (entity.entityType === 'base') {
      console.warn('MovementSystem: Base cannot move', entityId)
      return false
    }
    if (entity.entityType === 'alert') {
      console.warn('MovementSystem: Alert cannot move', entityId)
      return false
    }

    const currentX = (entity.gameX ?? 0) as number
    const currentY = (entity.gameY ?? 0) as number
    const distance = Math.hypot(targetX - currentX, targetY - currentY)

    // Don't queue moves shorter than threshold
    if (distance < 1) return false

    this.moveCommands.set(entityId, {
      targetX,
      targetY,
      speed: speed ?? this.getDefaultSpeed(entity),
      distance,
      progress: 0
    })

    return true
  }

  /**
   * Get default movement speed based on entity type
   * @param entity - Entity object
   * @returns Default speed
   */
  getDefaultSpeed (entity: { vehicleType?: string, type?: string }): number {
    const type = entity.vehicleType || entity.type || 'scout'
    const speeds: Record<string, number> = {
      scout: 60,
      tank: 40,
      transport: 50,
      raider: 70,
      hostile: 55,
      static: 0
    }
    return speeds[type] ?? 50
  }

  /**
   * Get entity by ID from state
   * @param entityId - Entity ID
   * @returns Entity object or null
   */
  getEntity (entityId: number): { vehicleType?: string, type?: string, gameX?: number, gameY?: number, entityType?: string, movement?: { canMove?: boolean } } | null {
    const entities = this.gameEngine.state.get('entities') as Map<number, { vehicleType?: string, type?: string, gameX?: number, gameY?: number, entityType?: string, movement?: { canMove?: boolean } }>
    if (!(entities instanceof Map)) return null
    return entities.get(entityId)
  }

  /**
   * Process all pending movement commands
   * @param dt - Delta time in seconds
   */
  processMovements (dt: number): void {
    if (this.moveCommands.size === 0) return

    const entities = this.gameEngine.state.get('entities') as Map<number, { gameX?: number, gameY?: number }>
    let changed = false

    for (const [entityId, command] of this.moveCommands) {
      const entity = entities.get(entityId)
      if (!entity) {
        this.moveCommands.delete(entityId)
        continue
      }

      // Calculate movement
      const currentX = (entity.gameX ?? 0) as number
      const currentY = (entity.gameY ?? 0) as number
      const dx = command.targetX - currentX
      const dy = command.targetY - currentY
      const distance = Math.hypot(dx, dy)

      if (distance <= 0) {
        this.moveCommands.delete(entityId)
        continue
      }

      // Move towards target
      const moveDistance = command.speed * dt
      if (moveDistance >= distance) {
        // Arrived at target
        entity.gameX = command.targetX
        entity.gameY = command.targetY
        this.moveCommands.delete(entityId)
        changed = true
      } else {
        // Continue moving
        const progress = moveDistance / distance
        entity.gameX = currentX + dx * progress
        entity.gameY = currentY + dy * progress
        changed = true
      }
    }

    if (changed) {
      this.gameEngine.state.merge({ entities }, 'entitiesUpdated')
    }
  }

  /**
   * Clear all pending move commands for an entity
   * @param entityId - Entity ID
   */
  clearMoveCommand (entityId: number): void {
    this.moveCommands.delete(entityId)
  }

  /**
   * Clear all pending move commands
   */
  clearAllCommands (): void {
    this.moveCommands.clear()
  }

  update (dt: number): void {
    this.processMovements(dt)
  }

  render (): void {
    // Movement doesn't render anything directly
    // RendererSystem will render updated positions
  }

  destroy (): void {
    if (this.isDestroyed) return
    this.isDestroyed = true
    this.moveCommands.clear()
    this.gameEngine = null as unknown as GameEngine
  }
}

export function createMovementSystem (gameEngine: GameEngine): MovementSystem {
  return new MovementSystem(gameEngine)
}
